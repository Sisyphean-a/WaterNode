...
本项目旨在开发一款基于 Flutter 的双端（Android / Windows）内部测试管理工具，用于模拟和管理多账号环境下的物联网设备（IoT）交互。当前阶段的核心目标是完成统一控制台壳层重构：首页仅承担信息展示、运行快照和取水入口，批量任务、终端管理、凭证管理与登录授权通过左侧边栏分类进入。
...
- **账号接入：** 模拟手机号与验证码登录，获取测试 Token。
  ...
  **壳层入口：** 账号相关功能收敛在 `账号中心`，包含 `凭证管理` 和 `登录授权` 两个页面；凭证管理页内提供“新增登录凭证”入口跳转到登录授权页。
...
- **页面承载：** 批量任务能力统一放入 `任务中心 / 批量任务` 页面；首页仅显示最近一次运行快照，不再直接放置批量执行按钮。
...
- **统一控制台壳层 (Console Shell)**
  - 左侧：按 `总览中心 / 任务中心 / 设备中心 / 账号中心` 分组展示导航。
  - 窄屏：使用 Drawer。
  - 宽屏：使用可折叠侧边栏，折叠态保留图标并通过 tooltip / 语义标签补充可访问性。
- **Page 1: 首页概览 (Dashboard)**
  - 顶部：账号池状态卡片（总账号数、在线账号、总积分池）。
  - 中部：`取水操作` 卡片，提供【立即取水】入口，点击后跳转终端大厅。
  - 下部：`运行快照`，展示账号载入情况和最近一条批处理日志。
- **Page 2: 批量任务 (Task Center)**
  - 提供【执行批量打卡】、【执行批量积分抽取】按钮。
  - 展示完整任务执行日志。
- **Page 3: 终端管理大厅 (Device Station)**
  - 顶部：区域级联选择器。
  - 列表：展示 IoT 终端卡片（设备名称、设备 ID、在线状态）。
  - 操作：每个卡片附带【下发指令】测试按钮。
- **Page 4: 凭证管理 (Credential List)**
  - 列表展示：手机号、当前积分余额、有效状态。
  - 页面内按钮：【新增登录凭证】。
- **Page 5: 登录授权页 (Auth Page)**
  - 表单：手机号、验证码输入框及获取按钮。
  - 登录成功后回到 `凭证管理`。
