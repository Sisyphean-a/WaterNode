...
class DispatchWorkbenchSection extends StatelessWidget {
  ...
  Widget _buildContent(BuildContext context, ThemeData theme) {
    ...
    final selectedCredential =
        credentialController.credentials.firstWhereOrNull(
          (item) => item.mobile == selectedCredentialMobile,
        ) ??
        deviceController.selectedCredential.value;
    ...
    final accountLabel = selectedCredential == null
        ? '未选中任何账号'
        : _accountLabel(selectedCredential);
    final stationLabel = selectedStation == null
        ? '未绑定对应设备'
        : _stationLabel(selectedStation);
    ...
    return Column(
      ...
      children: [
        ...
        _StatusCardTile(
          label: '当 前 账 号',
          value: accountLabel,
          trailingText: selectedCredential != null
              ? '${selectedCredential.points} 分'
              : null,
  ...
  void _showAccountSelector(BuildContext context) {
    final accounts = credentialController.credentials
  ...
  Future<void> _confirmAndSend(
    ...
    DeviceStation detail;
    try {
      detail = await deviceController.fetchStationDetail();
    } catch (error) {
      deviceController.lastError.value = error.toString();
      return;
    }
    final stationName = detail.name;
    final stationStatus = _confirmStationStatus(detail);
  ...
  int? _estimatedCost(int quantity, FreeWaterConfig? config) {
    if (config == null) {
      return null;
    }
    return config.beanValue * quantity;
  }

  String? _confirmStationStatus(DeviceStation station) {
    final statusDescription = station.statusDescription?.trim();
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Icon(
          icon,
          size: emphasize ? 48 : 36,
          color: emphasize ? null : theme.colorScheme.primary,
        ),
        const SizedBox(height: 12),
        Text(
          label,
